...
struct MediaPacketSource : public PacketSource {
    MediaPacketSource()
        ...
        CHECK_EQ((status_t)OK,
                 mExtractor->setDataSource(NULL /* httpService */, MEDIA_FILENAME));
...
void TestHandler::listen() {
    sp<AMessage> msg = new AMessage(kWhatListen, this);
    msg->post();
}
...
void TestHandler::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatListen:
        {
            sp<AMessage> notify = new AMessage(kWhatTimeSyncerNotify, this);
            mTimeSyncer = new TimeSyncer(mNetSession, notify);
            looper()->registerHandler(mTimeSyncer);

            notify = new AMessage(kWhatReceiverNotify, this);
        ...
        case kWhatConnect:
        {
            ...
            notify = new AMessage(kWhatSenderNotify, this);
...
status_t TestHandler::readMore() {
    ...
    sp<AMessage> msg = new AMessage(kWhatSendMore, this);
